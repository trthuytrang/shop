...
export default function DashboardNewsCreate(props) {
    ...
    useEffect(()=> {
        if (news) {
            ...
            axios.get(`${process.env.REACT_APP_END_POINT}/news`)
                .then(res => {
    ...
    const onSubmit = (event) => {
        ...
        axios.post(`${process.env.REACT_APP_END_POINT}/news/update/${news._id}`, formData, config)
        .then(()=>{
    ...
    const deleteImg = (event) => {
        ...
        items.splice(id, 1)
        setNewsImg(items)
        axios.post(`${process.env.REACT_APP_END_POINT}/news/update/${news._id}`, {
            deleteImgId: id
        })
    }
