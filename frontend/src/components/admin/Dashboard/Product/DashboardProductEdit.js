...
export default function DashboardProductEdit(props) {
    ...
    useEffect(()=> { 
        if (product) {
            ...
            axios.get(`${process.env.REACT_APP_END_POINT}/category`)
                .then(res => {
                    setCate(res.data)
                }
            )
            axios.get(`${process.env.REACT_APP_END_POINT}/products`)
                .then(res => {
    ...
    const onSubmit = (event) => {
        ...
        axios.post(`${process.env.REACT_APP_END_POINT}/products/update/${product._id}`, formData, config)
        .then(()=>{
    ...
    const addNewCate = () => {
        axios.post(`${process.env.REACT_APP_END_POINT}/category`, {
    ...
    const deleteImg = (event) => {
        ...
        items.splice(id, 1)
        setProductImg(items)
        axios.post(`${process.env.REACT_APP_END_POINT}/products/update/${product._id}`, {
            deleteImgId: id
        })
    }
