...
export default function DashboardBody(props) {
    ...
    useEffect(()=> {
        Axios.get(`${process.env.REACT_APP_END_POINT}/products/${props.productId}`)
            .then(res => {
                setProduct(res.data)
            } 
        )
        Axios.get(`${process.env.REACT_APP_END_POINT}/news/${props.productId}`)
            .then(res => {
                setNews(res.data)
            } 
        )
        Axios.get(`${process.env.REACT_APP_END_POINT}/users/list/${props.productId}`)
            .then(res => {
                setUser(res.data)
            } 
        )
        Axios.get(`${process.env.REACT_APP_END_POINT}/order/${props.productId}`)
            .then(res => {
                setOrder(res.data)
            } 
        )
        Axios.get(`${process.env.REACT_APP_END_POINT}/collection/${props.productId}`)
            .then(res => {
                setCollection(res.data)
            } 
        )
        Axios.get(`${process.env.REACT_APP_END_POINT}/email/${props.productId}`)
            .then(res => {
                setEmail(res.data)
            } 
        )
